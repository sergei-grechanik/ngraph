...
public:
    ...
    json serialize_output(const Output& output);
...
public:
    ...
    Output deserialize_output(json j);
...
Output JSONDeserializer::deserialize_output(json j)
{
    size_t index;
    ...
    return Output(deserialize_node_reference(json_node_reference), index);
}
...
struct OutputHelper
{
    OutputHelper(const Output& output)
        : m_output(output)
    {
    }

    operator shared_ptr<Node>() const { return get_output_element(m_output); }
    operator const Output&() const { return m_output; }
    Output m_output;
};
...
struct OutputVectorHelper
{
    ...
    void push_back(const Output& output) { m_vector.push_back(output); }
...
// #pragma GCC diagnostic error "-Wimplicit-fallthrough"
        ...
        case OP_TYPEID::GetOutputElement:
        {
            node = make_shared<op::GetOutputElement>(
                static_cast<Output>(args[0]).get_node_shared_ptr(), node_js.at("n").get<size_t>());
            break;
        }
...
json JSONSerializer::serialize_output(const Output& output)
{
    json result;
...
{
    json result;
    for (const Output& output : output_vector)
    {
