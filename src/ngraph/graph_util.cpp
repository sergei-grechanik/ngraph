    ...
    ngraph::clone_nodes(const std::list<std::shared_ptr<ngraph::Node>>& nodes, NodeMap& node_map)
{
    ...
    for (auto node : sorted_nodes)
    {
        if (node_map.count(node.get()) == 0)
        {
            ...
            {
                Output output = input.get_source_output();
...
bool ngraph::is_equal_to_const_value(std::string const_value, const Output& reduce_constant)
{
                                          ...
                                          const shared_ptr<Node>& dst_node)
{
    ...
    std::vector<Input> dst_inputs = get_inputs_from(*src_node, *dst_node);
    ...
    std::vector<Output> src_outputs = get_outputs_to(*src_node, *dst_node);
...
bool ngraph::is_zero(const Output& reduce_constant)
{
...
std::vector<Input> ngraph::get_inputs_from(Node& src, Node& dst)
{
...
std::vector<Output> ngraph::get_outputs_to(Node& src, Node& dst)
{
