...
shared_ptr<Node> builder::reshape(const Output& value, const Shape& shape)
{
    return make_shared<op::Reshape>(value, get_default_order(value.get_shape().size()), shape)
        ->add_provenance_group_members_above({value});
}

shared_ptr<Node> builder::reorder_axes(const Output& value, vector<size_t> axes_order)
{
...
shared_ptr<Node> builder::transpose(const Output& value)
{
...
shared_ptr<Node> builder::flatten(const Output& value, int axis)
{
...
shared_ptr<Node> builder::flatten(const Output& value, const Output& axis)
{
...
shared_ptr<Node> builder::squeeze(const Output& value, vector<size_t> axes)
{
    if (axes.empty())
    {
...
shared_ptr<Node>
    builder::collapse(const Output& value, const size_t start_axis, const size_t end_axis)
{
...
shared_ptr<Node> builder::expand_dims(const Output& value, size_t axis)
{
