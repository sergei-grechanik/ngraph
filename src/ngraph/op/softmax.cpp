...
op::v0::Softmax::Softmax(const Output& arg, const AxisSet& axes)
    : Op({arg})
{
    set_argument(
        1,
...
op::v0::Softmax::Softmax(const Output& arg, const Output& axes)
    : Op({arg, axes})
{
    constructor_validate_and_infer_types();
}
...
op::v1::Softmax::Softmax(const Output& arg, const size_t axis)
    : Op({arg})
    , m_axis(axis)
{
    constructor_validate_and_infer_types();
