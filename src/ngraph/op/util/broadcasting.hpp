...
namespace ngraph
{
    namespace op
    {
        ...
        std::shared_ptr<Node> numpy_style_broadcast(const Output& value, const Shape& shape);
        ...
        inline std::shared_ptr<Node> make_broadcast_node(const Output& output, Shape new_shape)
        {
            return std::make_shared<op::Broadcast>(
                output, new_shape, calculate_broadcast_axes(new_shape, output.get_shape()));
        }

        inline std::shared_ptr<Node> make_broadcast_node(const Output& value,
