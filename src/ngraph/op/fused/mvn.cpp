...
op::MVN::MVN(const Output& data, bool across_channels, bool normalize_variance, double eps)
    : FusedOp({data})
    , m_eps{eps}
...
op::MVN::MVN(const Output& data, AxisSet reduction_axes, bool normalize_variance, double eps)
    : FusedOp({data})
    , m_eps{eps}
...
NodeVector op::MVN::decompose_op() const
{
    ...
    else
    {
        ...
        auto eps_node = op::Constant::create(
            data.get_element_type(), Output(variance).get_shape(), vector<double>{m_eps});
