...
op::v0::Split::Split(const Output& data, const Output& axis, const size_t num_split)
    : FusedOp({data, axis})
    , m_split_evenly{true}
...
op::v0::Split::Split(const Output& data, const Output& axis, const std::vector<size_t>& splits)
    : FusedOp({data, axis})
...
op::v0::Split::Split(const Output& data, int axis, const std::vector<size_t>& splits)
    : FusedOp({data})
    , m_split_evenly{false}
    , m_axis{axis}
    , m_num_split{0}
...
op::v1::Split::Split(const Output& data, const Output& axis, const size_t num_splits)
    : FusedOp({data, axis})
