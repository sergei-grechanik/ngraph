...
op::CrossEntropy::CrossEntropy(const Output& arg1,
                               const Output& arg2,
                               bool soft_label,
...
static Output get_2d_tensor(Output node)
{
...
static std::shared_ptr<Node> expand_shape(std::shared_ptr<Node> result, Output original)
{
...
static std::shared_ptr<ngraph::Node> create_mask(Output labels, Output input, int64_t ignore_index)
{
    auto mask_constant =
...
NodeVector op::CrossEntropy::decompose_op() const
{
    ...
    auto create_xe = [&](const Output& one_hot, const Output& input) {
...
op::CrossEntropyBackprop::CrossEntropyBackprop(const Output& input,
                                               const Output& labels,
                                               const Output& delta,
                                               bool soft_label,
                                               int64_t ignore_index)
