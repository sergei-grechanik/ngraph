...
shared_ptr<Node> op::PartialSlice::copy_with_new_args(const OutputVector& new_args) const
{
    if (new_args.size() != 1)
    {
...
shared_ptr<Node> op::PartialSliceBackprop::copy_with_new_args(const OutputVector& new_args) const
{
    if (new_args.size() != 2)
    {
        throw ngraph_error("Incorrect number of new arguments");
