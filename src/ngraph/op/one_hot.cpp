...
op::v0::OneHot::OneHot(const Output& arg, const PartialShape& shape, size_t one_hot_axis)
    : Op({arg})
    , m_shape(shape)
...
op::v1::OneHot::OneHot(const Output& indices,
                       const Output& depth,
                       const Output& on_value,
                       const Output& off_value,
                       int64_t axis)
