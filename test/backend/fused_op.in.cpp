...
NGRAPH_TEST(${BACKEND_NAME}, conv_bias_bprop_2d)
{
    ...
    auto conv_bprop = make_shared<op::ConvolutionBiasBackpropFiltersBias>(data,
                                                                          ...
                                                                          CoordinateDiff{0, 0},
                                                                          Strides{1, 1});
    auto output0 = conv_bprop->output(0);
    auto output1 = conv_bprop->output(1);
    auto f0 = make_shared<Function>(OutputVector{output0, output1}, ParameterVector{data, delta});
...
NGRAPH_TEST(${BACKEND_NAME}, lstm_cell_no_bias_no_peepholes)
{
    ...
    auto ht_function = make_shared<Function>(lstm_cell->output(0),
    ...
    auto ct_function = make_shared<Function>(lstm_cell->output(1),
                                             ParameterVector{X, H_t, C_t, W, R});
